...
def github_push(message=None):
    ...
    try:
        ...
        # ÉTAPE 1 : On ajoute TOUT d'abord (add .)
        subprocess.run(["git", "-C", GITHUB_REPO_PATH, "add", "."], check=True)
        
        # ÉTAPE 2 : On commit les changements locaux
        # On capture la sortie pour éviter que le script plante s'il n'y a rien à committer
        subprocess.run(["git", "-C", GITHUB_REPO_PATH, "commit", "-m", msg], capture_output=True)
        
        # ÉTAPE 3 : Maintenant on peut pull sans risque de conflit de "dirty worktree"
...
def add_job():
    ...
    if request.method == "POST":
        ...
        new_job = {
            ...
            "fullJobPosting": f.get("fullJobPosting","").strip(),
            "source":         "admin_web",
            "summary":        summary,
        }
